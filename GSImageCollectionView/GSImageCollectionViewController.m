...
- (void)tryPreloadPageIndex:(NSInteger)aPageIndex
{
    if (aPageIndex < self.dataSource.count) {
        ...
        if ([imageObject isKindOfClass:[NSString class]]) {
            [[SDWebImageManager sharedManager] cachedImageExistsForURL:[NSURL URLWithString:imageObject] completion:^(BOOL isInCache) {
                if (!isInCache) {
                    [[SDWebImageManager sharedManager] loadImageWithURL:[NSURL URLWithString:imageObject]
...
- (void)setDataSource:(NSArray *)dataSource
{
    ...
    NSMutableArray *filteredArray = [[NSMutableArray alloc]initWithObjects:@"",nil];
    for(int i = (int)[dataSource count] -1 ;i >= 0;i --){
        //containsObject 判断元素是否存在于数组中(根据两者的内存地址判断，相同：YES  不同：NO）
        if([filteredArray containsObject:[dataSource objectAtIndex:i]]) {
